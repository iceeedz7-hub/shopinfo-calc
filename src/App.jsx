...
const CANAIS = [
  { id: "kabum",       nome: "KaBuM!",        cor: "#FF6500", bg: "#1a0d00" },
  { id: "shopee",      nome: "Shopee",         cor: "#EE4D2D", bg: "#1a0800" },
  { id: "casasbahia",  nome: "Casas Bahia",    cor: "#0066CC", bg: "#00101f" },
  { id: "magalu",      nome: "Magazine Luiza", cor: "#0086FF", bg: "#00112a" },
  { id: "mercadolivre",nome: "Mercado Livre",  cor: "#FFE600", bg: "#1a1600" },
];
...
const C = {
  ...
  borderHover: "#333340",
  red: "#e8192c",
  redGlow: "rgba(232,25,44,0.18)",
  redDim: "rgba(232,25,44,0.08)",
  text: "#f0f0f5",
  textMid: "#8888a0",
  textDim: "#333344",
  green: "#22d47a",
...
const fmt = n => isNaN(n) ? "—" : n.toLocaleString("pt-BR", { style: "currency", currency: "BRL" });
const fmtPct = n => isNaN(n) ? "—" : `${n.toFixed(2).replace(".", ",")}%`;

function Field({ label, value, onChange, placeholder, unit, hint }) {
  const [focused, setFocused] = useState(false);
  return (
    <div>
      <label style={{ display:"block", fontSize:10, fontWeight:700, letterSpacing:"0.15em", textTransform:"uppercase", color:C.textMid, marginBottom:6, fontFamily:"'DM Sans',sans-serif" }}>{label}</label>
      <div style={{ position:"relative" }}>
        <input
          type="number" value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder||"0"}
          style={{ width:"100%", background:C.bg, border:`1px solid ${focused?C.red:C.border}`, borderRadius:10, padding:"11px 14px", color:C.text, fontSize:14, fontFamily:"'JetBrains Mono',monospace", outline:"none", transition:"all 0.2s", boxSizing:"border-box", boxShadow:focused?`0 0 0 3px ${C.redGlow}`:"none" }}
...
function ResultBox({ label, value, sub, color, big }) {
  return (
    <div style={{ background:C.bg, border:`1px solid ${C.border}`, borderRadius:12, padding:"14px 16px", flex:1 }}>
      <div style={{ fontSize:9, letterSpacing:"0.15em", color:C.textMid, fontWeight:700, marginBottom:6, fontFamily:"'DM Sans',sans-serif" }}>{label}</div>
      <div style={{ fontSize:big?22:16, fontWeight:800, color:color||C.text, fontFamily:"'JetBrains Mono',monospace", lineHeight:1 }}>{value}</div>
      {sub && <div style={{ fontSize:11, color:C.textMid, marginTop:4, fontFamily:"'JetBrains Mono',monospace" }}>{sub}</div>}
    </div>
  );
}

function calcular(form, canalId) {
  ...
  // Preço de venda mínimo para MC positiva (simplificado: custo / (1 - descontos%))
  const totalPct = (imposto + sga + comissao - rebate) / 100;
  const precoBase = totalPct < 1 ? (custo + taxaExtra) / (1 - totalPct) : 0;

  // Usamos o precoBase como referência de cálculo
  ...
  const lucroLiquido   = mc;
  const llPct          = mcPct;

  return { preco, custoTotal, mc, mcPct, lucroLiquido, llPct, valorComissao, valorImposto, valorSGA, valorRebate, taxaExtra };
}
...
export default function App() {
  ...
  const setCanal = (canalId, field, val) => setForm(f => ({
    ...f,
    canais: { ...f.canais, [canalId]: { ...f.canais[canalId], [field]: val } }
  }));

  const canalAtual = CANAIS.find(c => c.id === abaCanal);
  const res = calcular(form, abaCanal);
    ...
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"'DM Sans',sans-serif", color:C.text }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ::-webkit-scrollbar{width:3px;} ::-webkit-scrollbar-track{background:${C.bg};} ::-webkit-scrollbar-thumb{background:${C.red};border-radius:2px;}
        ...
        .result-row:hover{background:${C.surface2}!important;}
      `}</style>
      ...
      <div style={{ position:"fixed", top:0, left:0, right:0, height:300, pointerEvents:"none", zIndex:0,
        background:`radial-gradient(ellipse 80% 200px at 50% 0%, rgba(232,25,44,0.1) 0%, transparent 100%)` }} />
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:740, margin:"0 auto", padding:"40px 20px 80px" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:20, marginBottom:48 }}>
          {/* Logo */}
          <div style={{ width:52, height:52, borderRadius:14, background:"linear-gradient(135deg,#e8192c,#a00018)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, boxShadow:"0 4px 20px rgba(232,25,44,0.4)", fontSize:22, fontWeight:900, color:"#fff", fontFamily:"'DM Sans',sans-serif", letterSpacing:"-0.05em" }}>in</div>
          <div>
            ...
            <h1 style={{ fontSize:"clamp(22px,5vw,34px)", fontWeight:800, color:C.text, letterSpacing:"-0.03em", lineHeight:1 }}>
              Calculadora de<br/>
              <span style={{ color:C.red }}>Marketplace</span>
            </h1>
          </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
          ...
          {/* BLOCO 1 — PRODUTO */}
          <div className="fade" style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:18, padding:24, animationDelay:"0s" }}>
          ...
          <div className="fade" style={{ animationDelay:"0.08s" }}>
            ...
            <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:12 }}>
              {CANAIS.map(c => (
                <button key={c.id} className="canal-tab" onClick={()=>setAbaCanal(c.id)}
          ...
          {/* BOTÃO CALCULAR */}
          <button className="calc-btn" onClick={()=>setCalculado(true)} style={{ width:"100%", padding:"16px 32px", background:`linear-gradient(135deg,${C.red},#a00018)`, border:"none", borderRadius:14, color:"#fff", fontSize:14, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", cursor:"pointer", fontFamily:"'DM Sans',sans-serif", transition:"all 0.2s", boxShadow:`0 4px 24px ${C.redGlow}`, animationDelay:"0.16s" }}>
            ...
            <div className="fade" style={{ background:C.surface, border:`1px solid ${canalAtual.cor}44`, borderRadius:20, overflow:"hidden", boxShadow:`0 0 40px ${canalAtual.cor}15` }}>
              ...
              <div style={{ background:`linear-gradient(135deg,${canalAtual.cor}18,${canalAtual.cor}05)`, borderBottom:`1px solid ${canalAtual.cor}22`, padding:"24px 28px" }}>
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:16 }}>
                  ...
                  <span style={{ fontSize:10, letterSpacing:"0.2em", color:canalAtual.cor, fontWeight:700 }}>RESULTADO — {canalAtual.nome.toUpperCase()}</span>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:10 }}>
                  <div style={{ background:C.bg, borderRadius:14, padding:"18px 20px", border:`1px solid ${C.border}`, gridColumn:"1/-1", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
                    <div>
                      <div style={{ fontSize:10, letterSpacing:"0.15em", color:C.textMid, fontWeight:700, marginBottom:6 }}>PREÇO DE VENDA SUGERIDO</div>
                  ...
                  <div style={{ background:C.bg, borderRadius:14, padding:"16px 18px", border:`1px solid ${res.mc>=0?"rgba(34,212,122,0.2)":C.border}` }}>
                    ...
                    <div style={{ fontSize:24, fontWeight:800, color:res.mc>=0?C.green:C.red, fontFamily:"'JetBrains Mono',monospace", lineHeight:1 }}>{fmtPct(res.mcPct)}</div>
                    <div style={{ fontSize:13, color:C.textMid, fontFamily:"'JetBrains Mono',monospace", marginTop:4 }}>{fmt(res.mc)}</div>
                  </div>
                  ...
                  <div style={{ background:C.bg, borderRadius:14, padding:"16px 18px", border:`1px solid ${res.lucroLiquido>=0?"rgba(34,212,122,0.2)":C.border}` }}>
                    ...
                    <div style={{ fontSize:24, fontWeight:800, color:res.lucroLiquido>=0?C.green:C.red, fontFamily:"'JetBrains Mono',monospace", lineHeight:1 }}>{fmtPct(res.llPct)}</div>
                    <div style={{ fontSize:13, color:C.textMid, fontFamily:"'JetBrains Mono',monospace", marginTop:4 }}>{fmt(res.lucroLiquido)}</div>
                  </div>
                </div>
              </div>
              ...
              <div style={{ padding:"20px 28px" }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:2 }}>
                  {[
                    { label:"Custo do produto",    value:parseFloat(form.custo)||0,  pct:((parseFloat(form.custo)||0)/res.preco)*100, color:"#3b82f6" },
          ...
          {calculado && !res && (
            <div style={{ background:C.surface, border:`1px solid ${C.red}33`, borderRadius:16, padding:"24px", textAlign:"center" }}>
              <p style={{ color:C.red, fontSize:14 }}>Preencha o custo do produto para calcular.</p>
            </div>
          )}
        </div>

        <p style={{ textAlign:"center", color:C.textDim, fontSize:11, marginTop:40, fontFamily:"'JetBrains Mono',monospace" }}>
          shopinfo · calculadora de marketplace
        </p>
      </div>
    </div>
  );
